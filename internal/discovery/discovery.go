...
func (d *Discoverer) FetchClusterHealth(ctx context.Context) (map[string]health.ClusterHealth, error) {
	url := d.adminURL + "/clusters?format=json"
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
	if err != nil {
		return nil, fmt.Errorf("create cluster health request: %w", err)
	}

	resp, err := d.client.Do(req)
	if err != nil {
		return nil, fmt.Errorf("fetch cluster health: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("cluster health returned status %d", resp.StatusCode)
	}

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("read cluster health body: %w", err)
	}
...
func (d *Discoverer) Watch(ctx context.Context) <-chan DiscoveryResult {
	...
	go func() {
		...
		send := func() {
			...
			if prevListeners == nil || listenersChanged || healthChanged {
				if listenersChanged {
...
// discoverFull fetches config_dump and cluster health in one pass.
func (d *Discoverer) discoverFull(ctx context.Context) (DiscoveryResult, error) {
	url := d.adminURL + "/config_dump"
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
	if err != nil {
		return DiscoveryResult{}, fmt.Errorf("create request: %w", err)
	}
	resp, err := d.client.Do(req)
	if err != nil {
		return DiscoveryResult{}, fmt.Errorf("fetch config dump: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return DiscoveryResult{}, fmt.Errorf("config dump returned status %d", resp.StatusCode)
	}
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return DiscoveryResult{}, fmt.Errorf("read config dump body: %w", err)
	}

	listeners, listenerClusters, err := d.parseConfigDump(body)
	if err != nil {
		return DiscoveryResult{}, err
	}

	clusterHealth, err := d.FetchClusterHealth(ctx)
	if err != nil {
		d.logger.Warn("failed to fetch cluster health, treating as unknown", "err", err)
